...
import { X, User, Mail, Briefcase, ChevronRight, ChevronDown, OctagonX, Search, Trash2, MapPin, DollarSign, Target, Zap, Users, Loader2, Palette } from "lucide-react"; 
 ...
 *
 * CHANGES:
 * 1. Removed "Type Filter" dropdown.
 * 2. Added "Colorize" toggle button in its place.
 * 3. Updated NodeCard to accept a `colorize` prop and change its background color dynamically based on the role `type`.
 */

const SZ = {
  NODE_W: 280, // CRITICAL: This must match the width inside NodeCard
  NODE_H: 110, // Matches the new taller height
  H_GAP: 20,   // INCREASED: This creates the white space between nodes
  V_GAP: 40,   // Vertical space between manager and direct reports
  PADDING: 0 
};
const MIN_ZOOM = 0.4;
const MAX_ZOOM = 2.5;
...
const NodeCard = React.memo(function NodeCard({
  ...
  const faded = !isMatch;
  // In Glean style, we can afford to show details earlier because the card is wider
  const showDetails = zoom > 0.5; 

  // --- 1. Role Logic (Icon & Color) ---
  ...
  const flag = getFlag(location); // You need to ensure getFlag is accessible here
  ...
  return (
    <div
      ...
      style={{
        left: x,
        top: y,
        width: 280, // SZ.NODE_W (Updated for Horizontal Layout)
        height: 110, // SZ.NODE_H (Updated to fit bottom pills)
        borderLeftWidth: colorize ? 4 : 1,
        borderLeftColor: (colorize && RoleConfig) ? '' : undefined 
      }}
      // Dynamic border color via ref to avoid massive tailwind class lists
      ref={el => {
          if (el && colorize && RoleConfig) {
             // Quick hack to map tailwind classes to real colors for the inline style
    ...
    >
      ...
      <div className="ml-3 mt-3 relative shrink-0">
        ...
        {/* Team Size Badge (Overlapping Avatar) */}
        {reporteesCount > 0 && (
      ...
      <div className="flex flex-col flex-1 min-w-0 pl-3 pr-2 py-2.5 h-full justify-between">
        ...
        <div>
            <div className="flex justify-between items-start">
                <h3 className="text-sm font-bold text-gray-900 truncate w-36 leading-tight" title={displayName}>
                    {displayName}
                </h3>
            ...
            <p className="text-[11px] text-gray-500 truncate mt-0.5 w-48" title={designation}>
                {designation || "No Title"}
            </p>
            
            {/* Email (Shown only if highly zoomed or explicit need, usually hidden in Glean cards until hover) */}
            <p className="text-[10px] text-gray-400 truncate mt-0.5">{email}</p>
        </div>

        {/* Bottom Section: Badges/Pills */}
        {showDetails && (
            <div className="flex flex-wrap items-center gap-1.5 mt-1">
                ...
                {/* 3. Awareness (If Go Ahead, show Dot, else show Text) */}
    ...
    <div className="w-full h-full p-6 select-none">
      ...
      <div className="flex gap-2 overflow-x-auto whitespace-nowrap py-1 mb-3 items-center">
        ...
        <button
            ...
            className={`px-2 py-1 rounded text-xs h-7 border flex items-center gap-1 transition-colors shrink-0 ${
                colorizeNodes 
      ...
      <div className={`fixed right-0 top-0 h-full z-50 transform transition-transform duration-300 ease-out ${drawerOpen ? "translate-x-0" : "translate-x-full"}`} style={{ width: 380 }}>
        <div className="h-full bg-white border-l shadow-2xl flex flex-col">
          ...
          <div className="p-4 overflow-y-auto flex-1 space-y-3 relative">
            ...
            <MultiEmailSelect label="Reporting To" placeholder="Type email, press Enter" value={form.reportingTo} onChange={(arr) => setForm({ ...form, reportingTo: arr })} suggestions={(existingCustomers || []).map(c => ({ email: c.email, name: c.name, label: c.name ? `${c.name} <${c.email}>` : c.email }))} />
            <MultiEmailSelect label="Reportees" placeholder="Type email, press Enter" value={form.reportees} onChange={(arr) => setForm({ ...form, reportees: arr })} suggestions={(existingCustomers || []).map(c => ({ email: c.email, name: c.name, label: c.name ? `${c.name} <${c.email}>` : c.email }))} />
...
function MultiEmailSelect({ label, value = [], onChange, suggestions = [], placeholder }) {
  ...
  const removeEmail = (email) => onChange((value || []).filter((x) => x !== email));
    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="border rounded-lg p-2">
        <div className="flex flex-wrap gap-1 mb-2">
          {(value || []).map((email) => (
            <span key={email} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 text-gray-800 text-xs">
              {email}
              <button onClick={() => removeEmail(email)} className="hover:text-red-600"><Trash2 size={11} /></button>
            </span>
          ))}
        </div>
        ...
        <div className="relative">
          ...
          <input className="w-full border rounded-lg px-8 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500" placeholder={placeholder} value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={handleKeyDown} />

          {query && filtered.length > 0 && (
            <div className="absolute z-10 mt-1 w-full bg-white border rounded-lg shadow max-h-48 overflow-auto">
              {filtered.map((s) => (
                <button key={s.email} type="button" onClick={() => addEmail(s.email)} className="w-full text-left px-3 py-2 hover:bg-gray-50">
