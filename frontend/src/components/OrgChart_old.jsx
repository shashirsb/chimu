// OrgChart.jsx — With Hover Tooltip for Log History
...
export default function OrgChart({
    ...
    // Canvas size
    const width = Math.max(layout.width + 40, 560) + SZ.PADDING * 2;
    const height =
        layout.height + 40 + ancestors.length * (SZ.NODE_H + SZ.V_GAP) + SZ.PADDING * 2;

    const rootY = 20 + ancestors.length * (SZ.NODE_H + SZ.V_GAP);
        ...
        <div className="w-full h-full overflow-auto">
            <div className="relative" style={{ width, height }}>
                ...
                {ancestors.map((node, idx) => {
                    ...
                    const y = 20 + idx * (SZ.NODE_H + SZ.V_GAP);
    ...
    return (
        <div
            className={`absolute rounded-xl border shadow-sm bg-white cursor-pointer select-none ${
                highlight ? "ring-2 ring-teal-400" : "border-gray-200"
            }`}
...
function buildScopedTree(customers, focusEmail, upDepth, downDepth) {
    ...
    const build = (n, depth) => {
        ...
        n.children = kids;
        kids.forEach(k => build(k, depth + 1));
    };
